        ...
        window.addEventListener('DOMContentLoaded', async function() {
            ...
            // كشف رابط الدعوة: ?invite=GROUPCODE
            const urlParams = new URLSearchParams(window.location.search);
            const inviteCode = urlParams.get('invite');
            if (inviteCode) {
                // ملء كود المجموعة تلقائياً في شاشة الدخول
                const codeInput = document.getElementById('group-code-input');
                if (codeInput) codeInput.value = inviteCode;
                // فتح مودال التسجيل مباشرة بعد تحميل Firebase
                setTimeout(() => openRegisterModal(inviteCode), 1000);
            }
        });
        ...
        window.openRegisterModal = function(inviteCode) {
            document.getElementById('register-modal').classList.add('active');
            const codeInput = document.getElementById('register-group-code');
            const code = inviteCode || currentGroupCode || 
                         document.getElementById('group-code-input')?.value?.trim() || '';
        ...
        window.switchPage = function(pageId) {
            ...
            // تحديث رابط الدعوة عند فتح صفحة الإعدادات
            if (pageId === 'page-settings') updateInviteLink();
        };
        ...
        function updateInviteLink() {
            ...
            if (!el || !currentGroupCode) return;
            const base = window.location.origin + window.location.pathname;
            const link = `${base}?invite=${currentGroupCode}`;
            el.textContent = link;
        }
        ...
        window.copyInviteLink = async function() {
            ...
            try {
                ...
                showAlert('✅ تم نسخ رابط الدعوة!', 'success');
            } catch {
                // fallback
